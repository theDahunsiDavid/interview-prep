...
AI_GATEWAY_API_KEY=your_key_here  # optional — enables automatic fallback
```
...
| AI orchestration    | Vercel AI SDK (`ai` v6)            | Provider-agnostic API with automatic fallback built in                  |
| Question generation | Llama 3.3 70B via Groq             | Strong reasoning, structured JSON output; falls back to Claude Haiku    |
| Answer evaluation   | Llama 3.3 70B via Groq             | Same model with same fallback; separate prompt with scoring rubric      |
...
**Model selection per task, with automatic fallback.** Each task (question generation, audio transcription, answer evaluation) gets its own model assignment in `lib/models.ts`. Changing a model for any task is a one-line edit. No prompt changes, no server action changes. Beyond provider abstraction, the app has automatic fallback built in: if Groq is unavailable (timeout, rate limit, 503), question generation and answer evaluation transparently fail over to Claude Haiku 4.5 via Vercel AI Gateway. The `withFallback` helper in `lib/ai.ts` provides this retry logic — the user sees no error, just a slightly slower response. Transcription is the one exception: it calls Groq's Whisper API directly with no fallback, since Whisper models are not available through the Gateway at this time.
...
Question generation and answer evaluation use the `withFallback` helper in `lib/ai.ts`. If the primary model (Groq's Llama 3.3 70B) returns a retryable error — timeout, network failure, rate limit, or 503 — the request is retried through Vercel AI Gateway using Claude Haiku 4.5. The user sees no error and no interruption.

To enable fallback, add an `AI_GATEWAY_API_KEY` to `.env.local`. Without it, the app runs on Groq alone and shows an error message if Groq is unavailable.

Transcription currently has no fallback — Whisper models are not available through Vercel AI Gateway. An `AbortSignal.timeout` of 120 seconds and per-status-code error messages handle the most common failure modes for that path.

To switch the primary provider entirely (e.g., from Groq to Google Gemini), edit the provider configuration in `lib/ai.ts` and update the model IDs in `lib/models.ts`. Server actions and prompts require no changes.
